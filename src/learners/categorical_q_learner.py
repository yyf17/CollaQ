...
class CateQLearner:
	...
	def get_comm_beta(self, t_env):
		...
		if self.args.is_comm_beta_decay and t_env > self.args.comm_beta_start_decay:
			comm_beta = comm_beta + 1. * (self.args.comm_beta_target - self.args.comm_beta) / \
	...
	def get_comm_entropy_beta(self, t_env):
		...
		if self.args.is_comm_entropy_beta_decay and t_env > self.args.comm_entropy_beta_start_decay:
			comm_entropy_beta = comm_entropy_beta + 1. * (self.args.comm_entropy_beta_target - self.args.comm_entropy_beta) / \
	...
	def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
		...
		if self.args.only_downstream or not self.args.use_IB:
			...
			expressiveness_loss = expressiveness_loss + (-th.log(label_prob + 1e-6)).sum() / mask.sum()
			...
			comm_loss = comm_loss * self.args.c_beta
			loss = loss + comm_loss
