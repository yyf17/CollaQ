...
class COMACritic(nn.Module):
    def __init__(self, scheme, args):
        ...
        has_rnn = getattr(self.args, "recurrent_critic", False)
        if has_rnn:
    ...
    def forward(self, batch, t=None):
        ...
        if self.rnn is not None:
            x = x.permute(0, 2, 1, 3).reshape(bs * n_agents, max_t, -1)   #   bs * n_agents,  max_t, vdim
            x, h_out = self.rnn(x)  # h0 defaults to 0 if not provided, TODO: make explicit
            x = x.reshape(bs, n_agents, max_t, -1).permute(0, 2, 1, 3)    #   bs, max_t, n_agents, vdim
