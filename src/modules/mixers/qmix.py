...
class QMixer(nn.Module):
    def __init__(self, args):
        ...
        # if self.args.get("hyper_initialization_nonzeros", 0) > 0:
        int_0_hyper_initialization_nonzeros = getattr(self.args, "hyper_initialization_nonzeros", 0)
        if int_0_hyper_initialization_nonzeros > 0:
            print('enter hyper_initialization_nonzeros')
        ...
        has_skip_connections = getattr(self.args, "skip_connections", False)
        ...
        # if self.args.get("gated",False):
        if getattr(self.args,"gated",False):
            print(self.args.gated)
            self.gate = nn.Parameter(th.ones(size=(1,)) * 0.5)

        self.non_lin = F.elu
        # if self.args.get("mixer_non_lin", "elu") == "tahh":
        if getattr(self.args,"mixer_non_lin", "elu") == "tahh":
            self.non_lin = F.tanh
    ...
    def forward(self, agent_qs, states):
        ...
        # Skip connections
        s = 0
        if self.args.git_root == "MAVEN":
            if getattr(self.args,"skip_connections",False):
