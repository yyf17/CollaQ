...
class EpisodeBatch:
    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            v = th.tensor(v, dtype=dtype, device=self.device)  #UserWarning: To copy construct from a tensor, it is recommended to use sourceTensor.clone().detach() or sourceTensor.clone().detach().requires_grad_(True), rather than torch.tensor(sourceTensor).
    ...
    def __getitem__(self, item):
        if isinstance(item, str):
            ...
            ret = EpisodeBatch(new_scheme, new_groups, self.batch_size, self.max_seq_length, data=new_data, device=self.device, git_root=self.git_root)
            return ret
        else:
            ...
            ret = EpisodeBatch(self.scheme, self.groups, ret_bs, ret_max_t, data=new_data, device=self.device, git_root=self.git_root)
            return ret
    ...
    def __repr__(self):
        return "EpisodeBatch. Batch Size:{} Max_seq_len:{} Keys:{} Groups:{} Git_root".format(self.batch_size,
                                                                                     self.max_seq_length,
                                                                                     self.scheme.keys(),
                                                                                     self.groups.keys(),
                                                                                     self.git_root)
...
#modify
#add two key parameters:
# - git_root            for DOP
# - burn_in_period      for QPLEX
class ReplayBuffer(EpisodeBatch):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu", git_root="", burn_in_period=None):
        super(ReplayBuffer, self).__init__(scheme, groups, buffer_size, max_seq_length, preprocess=preprocess, device=device, git_root=git_root)
    ...
    def can_sample(self, batch_size):
        if self.burn_in_period is not None:  #  for QPLEX
...
class Best_experience_Buffer(EpisodeBatch):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu", git_root="", burn_in_period=None):
        super(Best_experience_Buffer, self).__init__(scheme, groups, buffer_size, max_seq_length, preprocess=preprocess, device=device, git_root=git_root)
