...
class StarCraft2Set1Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        print('#' * 20)
        print('#' * 20)
        print('use smac in local dir, StarCraft2Set1Env')
        print('#' * 20)
        print('#' * 20)

        # Map arguments
        self.map_name = map_name
        print("self.map_name:",self.map_name)
    ...
    def _launch(self):
        ...
        if len(self.map_name) == 1:
            ...
            map_play_area_max = map_info.playable_area.p1
            self.max_distance_x = map_play_area_max.x - map_play_area_min.x
            self.max_distance_y = map_play_area_max.y - map_play_area_min.ymap_name
    ...
    def unit_sight_range(self, agent_id):
        ...
        # return 9
        # modify: 去掉sight，即去掉po
        return np.sqrt(self.map_x ** 2 + self.map_y ** 2)
    ...
    def get_obs_agent(self, agent_id):
        ...
        move_feats = np.ones(move_feats_len, dtype=np.float32)
        enemy_feats = np.ones((self.n_enemies, nf_en), dtype=np.float32)
        ally_feats = np.ones((self.n_agents - 1, nf_al), dtype=np.float32)
        own_feats = np.ones(nf_own, dtype=np.float32)
    ...
    def get_state(self):
        ...
        enemy_state = np.ones((self.n_enemies, nf_en))
