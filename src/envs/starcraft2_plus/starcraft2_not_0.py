...
class StarCraft2Not0Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        print('#' * 20)
        print('#' * 20)
        print('use smac in local dir, StarCraft2Not0Env')
        print('#' * 20)
        print('#' * 20)
    ...
    def get_obs_agent(self, agent_id):
        ...
        move_feats = np.ones(move_feats_len, dtype=np.float32) * -1
        enemy_feats = np.ones((self.n_enemies, nf_en), dtype=np.float32) * -1
        ally_feats = np.ones((self.n_agents - 1, nf_al), dtype=np.float32) * -1
        own_feats = np.ones(nf_own, dtype=np.float32) * -1
    ...
    def get_state(self):
        ...
        ally_state = np.ones((self.n_agents, nf_al)) * -1
        enemy_state = np.ones((self.n_enemies, nf_en)) * -1
