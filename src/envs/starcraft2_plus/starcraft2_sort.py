...
class StarCraft2SortEnv(MultiAgentEnv):
    ...
    def __init__(
        ...
        print('#' * 20)
        print('#' * 20)
        print('use smac in local dir, sort emeny by type')
        print('#' * 20)
        print('#' * 20)
    ...
    def get_obs_agent(self, agent_id):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id in range(self.n_enemies):
                e_unit = self.get_emeny_by_id(e_id)
                e_x = e_unit.pos.x
                e_y = e_unit.pos.y
    ...
    def get_obs(self):
        ...
        # agent_id = [i[-2:] for i in agents_obs]
        # other_id = [[list(i[4 + 8 * j + 6: 4 + 8 * j + 8]) for j in range(5)]for i in agents_obs ]
        # team_id = [[list(i[4 + 8 * 4 + 8 * j + 6: 4 + 8 * 4 + 8 * j + 8]) for j in range(4)] for i in agents_obs]
        # print(agents_obs[0])
        # print('agent id:', agent_id[0])
        # print('team id:', team_id[0])
        # print('other id:', other_id[0])
        return agents_obs
    ...
    def init_units(self):
        ...
        while True:
            ...
            # modify the init form, just random emeny id
            # for unit in self._obs.observation.raw_data.units:
            #     if unit.owner == 2:
